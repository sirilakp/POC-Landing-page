...
I have deployed a new ASP.NET Core web application to the Test Tenant (`inhazuretest.onmicrosoft.com`). The app runs on Azure App Service with a **system-assigned Managed Identity**. It uses **Microsoft Graph** to invite external users (B2B guests) and manage their application role assignments.
...
The deployment account (`adm_sirilakp@inhazuretest.onmicrosoft.com`) does not have any of these roles and cannot perform this step.
...
To avoid needing to escalate for this type of task in the future, I would also like to request that the **Application Administrator** directory role be assigned to:
...
|---|---|---|
| Controls access to | Azure resources (VMs, App Service, Key Vault, etc.) | Microsoft Entra ID data (users, groups, app roles) |
